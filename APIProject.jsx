...
export default class ContactAPIProject extends Component {
  ...
  getContactList = async () => {
    let resp = await fetch("http://localhost:3000/item")
    console.log(resp)
  ...
  addContact = async () => {
    ...
    let resp = await fetch("http://localhost:3000/item", headers)
    let result = await resp.json()
    this.getContactList()
    this.formEmpty()
  ...
  handleDelete = async (id) => {
    ...
    let response = await fetch(`http://localhost:3000/item/${id}`, headers)
